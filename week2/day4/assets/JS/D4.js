...
for (s = 0; s < starWarsCharacters.length; s++) {
  if (starWarsCharacters[s].gender === 'female') {
    femaleCharacters.push(starWarsCharacters[s])
  }
}
...
let f = 0;
let totalMass = 0;
while (f < starWarsCharacters.length) {
  totalMass += parseInt(starWarsCharacters[f].mass)
  f++
}
...
let pippo = starWarsCharacters [Math.floor(Math.random()*starWarsCharacters.length)]
